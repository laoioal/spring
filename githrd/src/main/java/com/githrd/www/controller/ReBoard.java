...
/*	
	@RequestMapping("/writeProc.blp")
	public ModelAndView writeProc(ModelAndView mv, HttpSession session, RedirectView rv, int nowPage, BoardVO bVO) {
		...
		int cnt = rDao.insertReboardcom(bVO);
		...
		if(cnt == 1) {
			...
			mv.addObject("NOWPAGE", nowPage);
			mv.addObject("VIEW", "/www/reBoard/reBoardWrite.blp");
		}
